    ...
    module Consultants
      class Resource < Grape::API
        ...
        get 'consultants' do
          ...
          @consultants ||= begin
              result = redis.hgetall(Consultant::REDIS_SCOPE).values
              result = result.map{ |value| Consultant.from_redis(value) }
        ...
        put 'consultants/:login', requirements: { login: /[a-z]+/ } do
          login      = params[:login]
          consultant = redis.hget(Consultant::REDIS_SCOPE, login) or error!({ error: 'NOT FOUND' }, 404)
          consultant = Consultant.from_redis(consultant)
          consultant.p2 = params[:p2] if params[:p2]
          consultant.p3 = params[:p3] if params[:p2]
          redis.hset(Consultant::REDIS_SCOPE, login, consultant.to_json)
          consultant
        end
      end
    end
