  ...
  module V1
    describe 'consultant api' do
      ...
      before(:each) do
        ...
        @redis.flushdb
        @data.each do |record|
          @redis.hset API::V1::Consultants::Consultant::REDIS_SCOPE, record['login'], record.to_json
        end
      end
      ...
      it 'should paginate the response' do
        @redis.flushdb
        13.times do |i|
          @redis.hset API::V1::Consultants::Consultant::REDIS_SCOPE, "u#{i}@example.com", { 'name' => "User#{i}", 'login' => "user#{i}" }.to_json
        end
      ...
      it 'should update the p2 and p3 attributes of a consultant' do
        ...
        consultant = JSON.parse(@redis.hget(API::V1::Consultants::Consultant::REDIS_SCOPE, 'jsmith'))
